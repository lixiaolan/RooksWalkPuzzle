...
public class ErrorLog {
    ...
    public CppMap<Integer, ErrorBundle> errorMap;
    ...
    public void setLog() {
	errorMap = new HashMap<Integer, ErrorBundle>;
	logErrors();
    }
	...
	else if (e.pointsPast) {
	    return TextureManager.PASSTHROUGHRULE;
	}
	else if (e.pointsAtBadDir) {
	...
	int index;
	int num;
	for (int i = 1; i < mBoard.tiles.length; i++ ) {
		...
		if (mBoard.tiles[i].getArrow().equals(TextureManager.LEFTARROW)) {
		    ...
		    //Checking if we point off the board
		    if (i/mBoard.boardHeight + j >= mBoard.boardWidth) {
			errorMap.get(i).pointsOffBoard = true;
		    }	
		}
